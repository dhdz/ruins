...
//get Cookie.get(name, [options])获取
// 获取 cookie 值。options 参数对象可选，取值如下：
...
// set Cookie.set(name, value, [options])添加
// 设置 cookie 值。参数对象 options 可选，可以有以下属性：
//path（字符串）、domain（字符串）、 expires（数值或日期对象）、raw（布尔值）。当 raw 为真值时，在设置 cookie 值时，不会进行 URI 编码

//remove Cookie.remove(name, [options]) 移除指定cookie
const Cookie = {}
